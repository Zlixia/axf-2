          ...
          <ul class="products-lists">
            ...
            :key="item.id">
              <router-link tag="div" class="products-list-img" :to="'/product-item/' + item.id">
                <img v-lazy="item.imgs.min" :ref="'dong' + item.id">
              </router-link >
  ...
  computed: {
    ...
    sortProducts () {
      if (this.sortType == '综合排序') {
        ...
        return this.activeCidProducts.slice(0).sort((a,b) => a.price - b.price)
      }else if (this.sortType === '价格最高') {
        return this.activeCidProducts.slice(0).sort((a,b) => b.price - a.price)
      }
    },
    //拿到用户信息
    user () {
      ...
      if (this.user.id) {
        ...
        this.$store.dispatch('addCart',product)
          .then(res => {
            ...
            */
            //开始
            let dongStart = this.$refs['dong' + product.id][0].getBoundingClientRect()
            //结束
