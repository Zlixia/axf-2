      ...
      <div class="main">
        ...
        <table class="user-news" v-show="(carts.length > 0)">
          <tr>
            ...
            <td>{{user.select_site.linkman}}&nbsp;&nbsp;{{user.select_site.sex == 1 ? '女士' : '先生'}}</td>
            <router-link tag="td" to="/site" rowspan="3" class="amend">修改</router-link>
          </tr>
          <tr>
            <th>电<span class="text-hidden">隐藏</span>话</th>
            <td>{{user.select_site.phone}}</td>
          </tr>
          <tr>
            <th>收货地址</th>
            <td>{{user.select_site.city}}{{user.select_site.site}} {{user.select_site.detailSite}}</td>
          </tr>
        </table>
        <!-- 用户信息下面的内容 -->
