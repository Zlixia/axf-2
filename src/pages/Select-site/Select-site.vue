<template>
  <div class="select-site">
    <HeaderGray back="true">
      <input slot="x" type="search" class="title-form" 
      placeholder="请输入地址" maxlength="10" 
      @input="bindsearch" v-model="searchKey">
    </HeaderGray>
    <div class="main">
      <!--百度地图容器-->
      ...
      <div class="pois-list">
        <ul>
          <li class="spline-bottom" v-for="(item, index) in results" :key="item.id" @click="addSite(item)">
      ...
      <div class="sug-list" v-show="searchResults.length > 0">
        <ul>
          <li class="spline-bottom" v-for="item in searchResults" 
          :key="item.street_id" @click="addSite(item)">
            <p class="pois-item-name">{{item.name}}</p>
            <p class="theme-font-gray">{{item.address}}</p>
          </li>
        </ul>
...
search?query=ATM机&tag=银行&region=北京&output=json&ak=您的ak //GET请求
*/
import _ from 'underscore'
import jsonp from 'jsonp'
import HeaderGray from '@/components/Header-gray/Header-gray'
import api from '@/api'
let ak = 'zXHFjDe4lm9B4MIo8LxLeurRNXus99Ul'
export default {
  components: {
    HeaderGray
  },
  ...
  activated (){
    ...
    let that = this
    //先判断是否选择了城市,再判断是否选择了地址
    if (this.selectedCity === '') {
      this.$msg("提示","您没有选择城市")
        .then(action => {
          if (this.$router === '/addsite') {
      ...
      if (this.selectedSite === '') {
        ...
        this.$http.get(api.host + '/citys')
          .then(res => {
            ...
            //对获取的城市列表进行循环，判断选中的城市和城市列表中的哪个城市相等
    ...
    function initBMap () {
      ...
      function addMapOverlay(){
        var markers = [
          {content:"",title:that.key,imageOffset: {width:-46,height:-21},
          position:{lat:that.y,lng:that.x}}
        ];
